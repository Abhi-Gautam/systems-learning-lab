...
def chapter_entries(g: dict) -> list[dict]:
    ...
    return [
        e for e in g["entries"] if e["level"] <= cl and not is_front_matter(e["title"])
    ]
...
def load_state() -> SchedulerState:
    if not STATE_PATH.exists():
        raise FileNotFoundError(
            f"No state at {STATE_PATH}. Run `scheduler.py init` first."
        )
...
def eligible_in_tier(
    tier: str, state: SchedulerState, on_slot_books: set[str]
) -> list[str]:
    out = []
...
def pick_next(tier: str, state: SchedulerState, on_slot_books: set[str]) -> str | None:
    ...
    return min(
        candidates,
        key=lambda b: (
            b not in STICKY_BOOKS,
            state.books[b].last_active,
            TIER_BOOKS[tier].index(b),
        ),
    )
...
def advance_one_day(
    state: SchedulerState, actual_ends: dict[str, int] | None = None
) -> dict:
    ...
    day_record = {
        "day": state.day,
        "date": str(START_DATE + timedelta(days=state.day)),
        "slots": {},
    }
    ...
    for tier in ("T1", "T2", "T3", "T4"):
        ...
        if finished_book:
            ...
            day_record["slots"][tier]["eviction"] = (
                f"book complete → {nxt or '(tier exhausted)'}"
            )
            if nxt:
...
def print_day(rec: dict) -> None:
    ...
    for tier in ("T1", "T2", "T3", "T4"):
        ...
        if s["book"] is None:
            ...
            continue
        flags = []
        if s.get("finished_chapter"):
            flags.append("✓chapter")
        if s.get("finished_book"):
            flags.append("★BOOK DONE")
        if s.get("yield"):
            flags.append(f"⇄ yield: {s['yield']}")
        if s.get("eviction"):
            flags.append(s["eviction"])
        flag_str = ("  [" + " · ".join(flags) + "]") if flags else ""
        print(
            f"  {tier} ({TIER_TIME[tier]:>2}min): {s['book']:<5} pp.{s['pages']:<10} "
...
def cmd_day(args) -> int:
    ...
    if args.n == 0:
        ...
        for tier in ("T1", "T2", "T3", "T4"):
            ...
            print(
                f"  {tier} ({TIER_TIME[tier]:>2}min): {b:<5} starting p.{state.books[b].page}  ({ch})"
            )
        return 0
    print_day(hist[-1])
    return 0
...
def cmd_status(args) -> int:
    ...
    live = (
        ...
        else " · no live state (simulation only)"
    )
    ...
    print(f"Gates: {GATES}")
    print(
        f"Quantum: soft yield day≥{SOFT_YIELD_DAY}+chapter, hard cap day {HARD_CAP_DAY}"
    )
    return 0
...
def cmd_init(args) -> int:
    if STATE_PATH.exists() and not args.force:
        print(
            f"State already at {STATE_PATH}; pass --force to overwrite.",
            file=sys.stderr,
        )
        return 2
    s = init_state()
...
def cmd_today(args) -> int:
    ...
    for tier in ("T1", "T2", "T3", "T4"):
        ...
        print(
            f"  {tier} ({TIER_TIME[tier]:>2}min): {slot.book:<5} read pp.{slot.page}-{end}  "
...
def cmd_override(args) -> int:
    ...
    if args.page is not None:
        ...
        book = s.slots[args.slot].book
        if book is not None:
            s.books[book].page = args.page
    save_state(s)
    print(
        f"Override applied; {args.slot} → {s.slots[args.slot].book} p.{s.slots[args.slot].page}"
    )
    return 0
...
def main() -> int:
    ...
    td = sub.add_parser(
        "today", help="(live) show today's slots without mutating state"
    )
    td.set_defaults(func=cmd_today)

    tk = sub.add_parser(
        "tick",
        help="(live) advance one day, persist, log. "
    ...
    ov = sub.add_parser(
        "override", help="(live) manually set slot's book/page when reality diverges"
    )
    ov.add_argument("slot", choices=("T1", "T2", "T3", "T4"))
    ov.add_argument("--book")
    ov.add_argument("--page", type=int)
    ov.set_defaults(func=cmd_override)

    rt = sub.add_parser(
        "rate", help="(live) calibrate per-book pages/session after real reading"
    )
    rt.add_argument("book")
    rt.add_argument("pages", type=int)
