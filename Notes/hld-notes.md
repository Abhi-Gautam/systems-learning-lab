...
## [2026-06-17] H03 · WhatsApp messaging · **Meta** · Discord · Snap · Telegram

- Connection management at 2B persistent sockets — why BEAM/Erlang holds ~2M/box where thread-per-conn JVM dies at ~50k
- Per-recipient message ordering — giving one inbox a total order when N senders write concurrently
- Headline bottleneck: the **session registry** (who owns the recipient's live socket *right now*) is read on every single message
...
1. Is the 100B/day uniform, or is there a 5–10× diurnal peak (evening, regional)? That sets the real connection-server fleet size, not the average.
2. Do we need *strict* total order per conversation, or just causal "messages from one sender arrive in send order"? The first needs a sequencer; the second is much cheaper.
3. E2E encryption — are we responsible for the Signal-protocol key exchange, or only for routing opaque ciphertext blobs we can't read? (Changes whether the server can dedup/rank.)
4. Multi-device: one account → N devices, all online. Does a message fan out to every device, and do read receipts sync across them?
5. Failure tolerance: is losing presence ("last seen") acceptable on a node crash, but losing an *undelivered message* never acceptable? (Yes → presence is soft state, messages are durable.)

### Back-of-envelope

```
Messages:    100B/day ÷ 86,400s ≈ 1.16M msg/s average
Peak:        ~5× diurnal      ≈ 5–6M msg/s
Connections: 2B users, ~50% online at peak ≈ 1B concurrent TCP sockets
Conn fleet:  1B ÷ 2M sockets/box ≈ 500 connection servers (+headroom → ~1–2k)
Payload:     ciphertext ~1KB avg (text); media is out-of-band (S3 + CDN), msg carries a pointer
Inbox write: 100B msg/day × ~1KB ≈ 100 TB/day raw; most delivered+acked within seconds,
             only undelivered (offline recipient) must persist → working set far smaller
Fan-out:     group of 1024 → 1 send becomes up to 1024 inbox writes (the group amplifier)
Memory:      1B sockets × ~10KB/conn state (TCP buffers + user/session) ≈ 10 TB RAM across fleet
```

The two numbers that drive the architecture: **1B concurrent idle sockets** (→ connection density) and **6M msg/s of routing** (→ session registry lookups).

### Functional / Non-functional requirements

| Functional | Non-functional |
|---|---|
| 1:1 + group chat (≤1024 members) | p99 delivery < 1s (online recipient) |
| Online/offline presence ("last seen") | 99.99% availability of message path |
| Read receipts (sent / delivered / read) | **Durability: an accepted message is never lost** |
| Offline delivery (store until recipient online) | Per-conversation ordering preserved |
| E2E encryption (server routes opaque blobs) | Presence is best-effort (soft state) |

### API contract

This is **not** an HTTP request/response system. The client holds a persistent bidirectional connection (WebSocket / XMPP-derived framing) and exchanges frames:

```
Client → Server  (over persistent socket):
  SEND     { client_msg_id, conversation_id, to, ciphertext, ts_client }
  ACK      { server_msg_id }                  // I received this from you
  READ     { conversation_id, up_to_msg_id }  // read receipt
  PRESENCE { state: online|typing|offline }

Server → Client  (pushed):
  DELIVER  { server_msg_id, conversation_id, from, ciphertext, seq }
  RECEIPT  { server_msg_id, status: delivered|read }
  PRESENCE { user, state, last_seen }
```

Idempotency: the `client_msg_id` (a UUID minted on the device) dedupes retries — a client that times out and re-SENDs the same message must not produce two inbox entries. The server maps `client_msg_id → server_msg_id` once and returns the same `server_msg_id` on replay.

### Data model

```
messages (per-conversation, append-only):
  conversation_id  (partition key)
  seq              (clustering key, monotonic per conversation)  ← total order lives here
  server_msg_id    (snowflake: time-ordered global id)
  sender_id
  ciphertext       (blob, opaque to server)
  created_at
  PRIMARY KEY ((conversation_id), seq)        // Cassandra/Scylla: wide row per conversation

inbox (per-recipient, undelivered queue):
  user_id          (partition key)
  server_msg_id    (clustering key)
  conversation_id
  delivered        (bool — cleared on ACK)

session_registry (soft state, in-memory + gossip):
  user_id   → { conn_server_id, socket_id, last_heartbeat }   // who owns the live socket
```

The `messages` table partitions by `conversation_id` so a single conversation is one wide row, range-scannable by `seq` for history. The `inbox` is the offline spool: written only when the recipient is *not* online, drained when they reconnect. The `session_registry` is the hot path — read on every message to find where to push.
...
flowchart LR
    U[Phone] --> DNS[GeoDNS]
    DNS --> AC[Anycast IP]
    AC --> LB[L4 LB - TCP, sticky]
    LB --> CS[Connection Server - holds the socket]
    CS --> REG[(Session Registry)]
    CS --> Q[Message Router / Queue]
    Q --> ST[(Message Store - Cassandra)]
    Q --> INB[(Inbox spool)]
    style CS fill:#ffe680,stroke:#b58900,stroke-width:2px
```

The component under study is the **Connection Server** — the box that terminates the persistent socket. Unlike a stateless app pod, it is *stateful*: it owns live sockets, so the L4 LB must route a reconnecting user consistently, and the registry must track which CS holds whom.

**Where "find the recipient's socket" lives** (the routing decision can sit at several layers):

| Layer | Sees | Latency cost | Blast radius | Best for |
|---|---|---|---|---|
| L4 LB (consistent hash on user) | IP, conn 5-tuple | <1ms | global | spreading sockets evenly |
| Session registry (central Redis) | user → CS mapping | 1–2ms | regional | authoritative routing |
| CS-local cache + gossip | recent peers | <0.5ms | per-box | hot conversations |
| Pub/sub by user_id (Kafka topic) | nothing — broadcast | 5–10ms | global | fallback when registry stale |

WhatsApp's real answer leans on the **registry + direct CS→CS forwarding**: CS-A looks up that the recipient is on CS-B, and forwards the frame straight to CS-B's socket.

### Architecture

```mermaid
flowchart TB
    S[Sender phone] -->|SEND frame, persistent socket| CSA[Conn Server A]
    CSA -->|1. assign seq, persist| ST[(Message Store)]
    CSA -->|2. lookup recipient| REG[(Session Registry)]
    REG -->|on CS-B| CSA
    CSA -->|3a. forward frame| CSB[Conn Server B]
    CSB -->|4. push DELIVER| R[Recipient phone]
    CSA -->|3b. recipient offline → spool| INB[(Inbox)]
    R -->|ACK| CSB --> CSA --> S
```

NFRs satisfied: **durability** at step 1 (persist before forwarding); **p99<1s** by direct CS→CS push (no polling); **offline delivery** at 3b; **ordering** by the `seq` assigned at step 1. E2E encryption: `ciphertext` is opaque end-to-end — the store and both CS boxes route bytes they can't decrypt.

### Deep dive — 2 mechanisms

#### Deep dive 1: Connection density — holding 2M sockets per box

**1. Why does this mechanism exist?**

The whole system is gated by one number: how many idle TCP connections can one machine hold? At 1B concurrent sockets, if a box holds 50k you need 20,000 boxes; if it holds 2M you need 500. That's a 40× fleet-cost difference, and it decides whether the product is economically viable. The rejected design is **thread-per-connection** (one OS thread blocked on `read()` per socket): a thread needs ~1MB of stack, so 50k threads = 50GB RAM in stacks alone, plus the kernel scheduler thrashing across 50k runnable threads. The constraint that forces the design: **a socket that is 99.9% idle must cost almost nothing while idle.**

**2. Walk it concretely.**

The mechanism is **event-driven I/O multiplexing** (epoll on Linux) + **lightweight green processes** (BEAM/Erlang, what WhatsApp actually used). One OS thread watches thousands of sockets via a single `epoll_wait()` syscall; only sockets with data ready wake up.

```mermaid
stateDiagram-v2
    [*] --> Idle: socket registered with epoll
    Idle --> Ready: bytes arrive (kernel marks fd readable)
    Ready --> Processing: epoll_wait returns this fd
    Processing --> Idle: frame handled, back to waiting
    Processing --> Closed: client disconnects
    Closed --> [*]
```

```
epoll memory walk (Linux), 1 box holding sockets:
  fd table entry per socket:        ~few KB (kernel sk_buff + TCP window buffers)
  epoll interest list:              1 epoll_event per fd  ≈ 12 bytes
  BEAM green process per conn:      ~2–4 KB heap (vs ~1MB OS thread stack)

  Round 1: epoll_wait(epfd, events, maxevents=10000, timeout) blocks
           → kernel returns only the 37 fds that have data (of 2,000,000 watched)
  Round 2: thread loops over 37 events, dispatches each to its green process
  Round 3: 1,999,963 idle sockets cost ZERO CPU this tick — they were never scanned
```

The key low-level fact: `epoll` is **O(ready)**, not O(watched). `select()`/`poll()` are O(watched) — they re-scan all 2M fds every call, which is why they cap out near 1k–10k. epoll keeps a kernel-side ready-list, so a tick that touches 37 active sockets does 37 units of work regardless of the 2M total.

**3. The cost you're accepting.**

| Property gained | Cost paid |
|---|---|
| 2M idle sockets ~free (epoll O(ready)) | Each box is now *stateful* — losing it drops 2M live sessions at once |
| Tiny per-conn memory (green process, not OS thread) | Must use a runtime built for it (BEAM/Go) — no blocking calls allowed on the I/O thread |
| No scheduler thrash (1 thread, N sockets) | A single slow handler (e.g. a sync DB call) head-of-line-blocks the event loop |
| Cheap fleet (500 boxes vs 20k) | Reconnect storms: if one box dies, 2M clients reconnect simultaneously → thundering herd on neighbors |

**4. Failure modes the interviewer will drill into.**

| Failure | What goes wrong | Mitigation |
|---|---|---|
| Connection server crashes | 2M sockets drop at once; those users appear offline | Clients auto-reconnect with **jittered backoff**; LB rehashes them across survivors; messages were already persisted so nothing is lost |
| Reconnect thundering herd | 2M clients hit survivors in the same second → CPU/accept-queue overload | Exponential backoff **with jitter** spreads reconnects over 30–60s; LB sheds with `SYN` cookies |
| epoll thread blocks on a syscall | One blocking `read()`/DB call freezes all 2M sockets on that box | Strict non-blocking I/O; offload any blocking work to a separate thread pool / async DB driver |
| File-descriptor exhaustion | `ulimit -n` default 1024 → `accept()` fails at 1024 sockets | Raise `fs.file-max` and per-process `RLIMIT_NOFILE` to millions; this is the #1 real-world gotcha |
| Slow-loris / half-open sockets | Idle sockets that never send accumulate, leaking memory | Heartbeat/keepalive timeout reaps dead sockets; presence doubles as liveness |

**5. How to derive it from first principles.**

1. State the trade-off: a chat app is **mostly idle sockets** — the cost model must make idle nearly free.
2. Thread-per-connection makes idle *expensive* (1MB stack each, scheduler overhead) → reject.
3. So one thread must watch many sockets → you need an I/O multiplexing syscall.
4. `select`/`poll` re-scan every fd per call → O(N) per tick, dies at ~10k → reject.
5. You need the kernel to tell you *only* which fds are ready → that is exactly `epoll` (Linux) / `kqueue` (BSD): O(ready) per tick.
6. Per-connection app state still needs a cheap container → a green process / goroutine (KBs), not an OS thread (MBs).
7. You've derived the event-loop + lightweight-process architecture — the BEAM model — without citing WhatsApp.

**6. Where this shows up in production.**

- **WhatsApp (Erlang/BEAM)**: famously reached **~2–3M TCP connections on a single box** (FreeBSD + tuned kernel), enabling ~50 engineers to serve ~900M users at acquisition.
- **Discord (Elixir/BEAM)**: gateway gRPC + WebSocket fan-out holds millions of concurrent connections; uses the same green-process model for guild/voice presence.
- **Linux epoll everywhere**: nginx, HAProxy, Node.js (libuv), Netty all sit on epoll/kqueue — the same O(ready) primitive that makes the C10M problem tractable.

#### Deep dive 2: Per-recipient message ordering

**1. Why does this mechanism exist?**

A conversation must read back in a single consistent order on every device. But N senders (in a group) write concurrently from different connection servers, with skewed clocks and variable network latency — so "order by wall-clock send time" is a lie (clocks drift tens of ms; a packet can arrive late). If two devices compute different orders, users see messages "reshuffle," and worse, a *reply* can appear before the message it answers. The rejected approach is **client timestamps as the sort key**: an attacker or a drifting clock can place a message in the past or future. The constraint: there must be **one authority** that stamps a total order per conversation, and that order must survive offline/reconnect.

**2. Walk it concretely.**

The mechanism is a **per-conversation monotonic sequence number** assigned by a single sequencer (the partition owner for that `conversation_id`). Every accepted message gets the next `seq`; clients sort by `seq`, not by clock.

```mermaid
sequenceDiagram
    participant A as Alice (CS-A)
    participant SEQ as Conv shard owner (seq=41)
    participant B as Bob (CS-B)
    A->>SEQ: SEND "hi"  (client_msg_id=u1)
    SEQ->>SEQ: seq := 42, persist
    SEQ-->>A: ACK seq=42
    SEQ->>B: DELIVER seq=42 "hi"
    A->>SEQ: SEND "you there?" (client_msg_id=u2)
    SEQ->>SEQ: seq := 43, persist
    SEQ->>B: DELIVER seq=43
    Note over B: Bob renders strictly by seq → 42,43 always
```

```
Sequence counter state for conversation_id=C7, across 3 sends:
  before:           {C7: last_seq = 41}
  Alice SEND u1  →  last_seq = 42   row (C7, 42, sender=alice, "hi")
  Bob   SEND b9  →  last_seq = 43   row (C7, 43, sender=bob,   "yo")     // concurrent, but serialized here
  Alice SEND u2  →  last_seq = 44   row (C7, 44, sender=alice, "?")

  Dedup map (idempotency): { u1→42, b9→43, u2→44 }
  Alice times out, re-sends u1 → owner sees u1 already mapped → returns seq=42, NO new row
```

The sequencer is the partition leader for `C7` (one Cassandra/Scylla wide-row owner, or a Raft group). Because all writes to `C7` funnel through one owner, the `last_seq` increment is **serialized** — that serialization *is* the total order. Concurrency between senders is resolved by whoever the owner processes first; there is no global clock involved.

**3. The cost you're accepting.**

| Property gained | Cost paid |
|---|---|
| Total order per conversation (no reshuffle) | A single owner per conversation = a write bottleneck for that conversation |
| Replay-safe dedup (client_msg_id → seq) | Must store the dedup map for a retry window (memory/TTL) |
| No clock dependency (immune to drift/spoof) | Cross-conversation order is undefined (only *within* a conversation) |
| Offline clients resync by `seq > last_seen_seq` | A 1024-member group funnels all writes through one owner → hot partition |

**4. Failure modes the interviewer will drill into.**

| Failure | What goes wrong | Mitigation |
|---|---|---|
| Sequencer (partition owner) crashes mid-increment | Did seq=43 commit before the crash? Risk of gap or dup | Persist `last_seq` in the same atomic write as the row (Raft log / LWT); on failover new owner reads max(seq)+1 |
| Client retries after timeout | Two rows for one logical message | `client_msg_id` dedup map returns the original `seq` — idempotent |
| Hot group (1024 members, everyone typing) | One conversation owner saturates | Group messages still get *one* seq, but **fan-out** to 1024 inboxes is parallelized off the critical path |
| Reconnect after 3 days offline | Client missed seqs 100–900 | Client sends `last_seen_seq=99`; server range-scans `seq>99` and replays in order |
| Two devices of same user disagree | Multi-device sees different order | Both sort by the server `seq`, never local clock → identical render |

**5. How to derive it from first principles.**

1. State the goal: every reader of conversation C must see the *same* order.
2. Try client timestamps → clocks drift and can be spoofed → order disagrees across devices → reject.
3. Try server receive-time → still multiple servers with skewed clocks + network jitter → reject.
4. Observe: a total order requires a *single point* that decides "this is #42, this is #43."
5. So funnel all writes for one conversation through one owner and hand out a monotonic counter — that counter is the order.
6. Make retries safe: map `client_msg_id → seq` so a re-send returns the same number (idempotent).
7. Make offline-resync trivial: clients store `last_seen_seq` and ask for everything greater.
8. You've derived a per-partition sequencer with idempotent dedup — without invoking vector clocks or wall time.

**6. Where this shows up in production.**

- **Kafka**: a partition has a single leader that assigns monotonically increasing **offsets** — the exact same "one owner hands out a counter = total order per partition" pattern; cross-partition order is undefined, just like cross-conversation here.
- **Discord (Cassandra/ScyllaDB)**: messages keyed by `(channel_id, message_id)` where `message_id` is a **Snowflake** (time-ordered), giving per-channel ordering with a globally sortable id; channels are the partition, mirroring conversations.
- **Signal / WhatsApp delivery**: the server assigns delivery order while message *contents* stay E2E-encrypted — ordering metadata is server-visible, payload is not, proving order and confidentiality are orthogonal.

### Bottlenecks + scaling levers

| Scale point | First bottleneck | Lever |
|---|---|---|
| 10× connections | Connection-server fleet RAM (sockets × per-conn state) | Add CS boxes; consistent-hash users; tune kernel `nofile`/buffers |
| 100× msg/s | Session registry read QPS (1 lookup per message) | CS-local cache of hot peers + gossip; pub/sub fallback; shard registry |
| 10× group size / hot group | Single conversation owner (sequencer) | Decouple seq-assignment (1 write) from fan-out (1024 inbox writes via queue) |
| 10× offline users | Inbox spool storage + reconnect drain | TTL on undelivered; batch-drain on reconnect; tier cold inboxes to cheap storage |

### Trade-off matrix

The key decision: **how does a sender find and reach the recipient's live socket?**

| Option | Latency | Cost | Complexity | Reliability |
|---|---|---|---|---|
| Poll a central inbox (HTTP) | ❌ (seconds) | ❌ (100B empty polls) | ✅ | ✅ |
| Central pub/sub by user_id (Kafka) | ⚠️ (5–10ms) | ⚠️ | ✅ | ✅ |
| Registry lookup + direct CS→CS forward — **chosen** | ✅ (<1ms hop) | ✅ | ❌ | ⚠️ (registry staleness) |

Chosen: **registry + direct forward**. Polling is disqualified by the BoE (100B empty polls/day). Pub/sub adds a hop and broadcast cost; direct CS→CS forwarding gives the lowest latency, and registry staleness is handled by a spool-on-miss fallback (if CS-B no longer holds the socket, the message lands in the inbox and is delivered on reconnect).

### Common follow-ups

1. **In-order delivery after a client reconnects offline?** — Client stores `last_seen_seq` per conversation; on reconnect it requests `seq > last_seen_seq`; server range-scans the wide row and replays in `seq` order. No clock involved.
2. **How does E2E encryption interact with group membership changes?** — Server routes opaque ciphertext; key management (Signal sender-keys) is client-side. On membership change, clients rotate the group key among themselves; the server only sees "blob to N recipients," never plaintext.
3. **1024 members all typing at once?** — Each message still gets *one* `seq` from the conversation owner (cheap), but the expensive 1024-way fan-out runs off the critical path via a queue, so the sequencer isn't blocked by delivery.
4. **Presence at 2B users?** — Presence is **soft state**: heartbeats refresh a TTL'd entry; on CS crash, presence simply expires (user shows offline) — never persisted, never blocks the message path.
5. **Read receipts without N² traffic?** — Coalesce: a `READ up_to seq=120` receipt acks everything ≤120 in one frame, instead of one receipt per message.

### What top-tier looks for

- **Mid-tier says**: "Use WebSockets and a message queue, store messages in Cassandra." Names the components, no numbers, no mechanism.
- **Staff-tier says**: "The fleet size is `1B sockets ÷ density`, and density is an epoll-vs-thread choice — O(ready) vs O(watched) — so I'd use an event-loop runtime to hit ~2M/box and 500 boxes. Ordering is a per-conversation sequencer (one owner hands out a monotonic `seq`, identical to a Kafka partition offset), with `client_msg_id` dedup for idempotent retries; clients sort by `seq`, never by clock, which is why two devices never disagree."

Why the difference: the mid-tier answer is a parts list; the staff answer **derives the two numbers that actually constrain the system** (connection density, routing QPS) and grounds each design choice in a kernel-level or distributed-systems primitive with both its upside and its bite.

### Diagnostic questions (self-quiz)

1. **Why does epoll scale to 2M sockets where poll() dies at 10k?** — wrong: "epoll is just faster." Right: epoll is O(ready) via a kernel ready-list; poll is O(watched), re-scanning all fds every call.
2. **Where does the total order of a conversation come from?** — wrong: "we sort by timestamp." Right: a single per-conversation sequencer hands out a monotonic `seq`; clocks are never the sort key.
3. **What makes a duplicate SEND idempotent?** — wrong: "we check if the text matches." Right: `client_msg_id → seq` dedup map returns the original `seq`, writing no new row.
4. **What happens to in-flight messages when a connection server crashes?** — wrong: "they're lost." Right: nothing is lost — messages are persisted before forwarding; clients reconnect and drain by `seq`.
5. **Why is presence handled differently from messages?** — wrong: "it's all just data in the DB." Right: presence is soft state (TTL'd heartbeat, expendable on crash); messages are durable (persist-before-deliver). Conflating them either makes presence too expensive or messages too lossy.

---

## [2026-06-10] H02 · Twitter home timeline · **Twitter** · Meta · LinkedIn · Snap

- Fan-out-on-write vs fan-out-on-read — and why neither alone survives the Obama problem
- Per-user materialized timeline in Redis: list-of-tweet-IDs sorted, capped, hydrated on read
- Headline trade-off: write amplification (push) vs read amplification (pull) — the hybrid threshold is the lever
...
**Clarifying questions to ask back before drawing anything:**
1. **Scale** — 500M users, 1B tweets/day given; what's the **read:write ratio**? (Assume 100:1 — 100B timeline-reads/day; this is what kills fan-out-on-read.)
2. **Consistency** — does "missed a tweet for 30s" violate the product? (Assume no — eventual consistency, ~5s ingest-to-fanout SLO; rules out synchronous fan-out.)
3. **Ranking** — strict reverse-chronological, or scored (ML rank)? (Assume reverse-chrono v1; ML rank is a re-rank stage on top, follow-up.)
4. **Retention** — how far back can a user scroll? (Assume 800 tweets cached per user ≈ 4 days at 200 follows; older = paginate from tweet store.)
5. **Failure tolerance** — region-level survival? (Assume active-active US-east + US-west; per-user timeline replicated 3x within region, async cross-region.)

### Back-of-envelope

```
Tweets:      1B / day = ~12k tweets/sec sustained, ~60k peak
Avg follows: 200 → fan-out write per tweet = 200 timeline writes avg
Fan-out QPS: 12k × 200 = 2.4M writes/sec to timeline store (avg)
Celebrities: top 0.01% (50k users) have >1M followers → 5% of tweets
             Obama (100M followers) × 1 tweet = 100M writes — 1 tweet OOMs the cluster
Reads:       500M users × 5 sessions/day × 1 timeline load = 2.5B reads/day
             = 29k reads/sec avg, ~150k peak
Timeline:    800 tweet IDs × 8B (id) = 6.4 KB per user
             500M users × 6.4 KB = 3.2 TB hot timeline cache → fits Redis sharded (~32 shards × 100GB)
Hydration:   200 tweet IDs × 400 B (tweet body) = 80 KB returned per timeline load
Bandwidth:   150k × 80 KB = 12 GB/s peak egress on tweet store reads
```

The two numbers that bound the design: **2.4M timeline writes/sec** (fan-out-write cost) and **150k timeline reads/sec** (fan-out-read budget). They cannot both be cheap.

### Functional / Non-functional requirements

| Functional | Non-functional |
|---|---|
| Show last ~800 tweets from followed users, reverse-chronological | p99 timeline load < 200ms |
| Refresh on pull-to-refresh (new tweets since last cursor) | Availability 99.95% — degrade gracefully (stale timeline acceptable, blank = fail) |
| Backfill on follow (new follow = followee's recent tweets appear) | Eventual consistency: ≤5s from tweet → follower's timeline (p99) |
| Mix in promoted tweets / ads at fixed positions | Durability: tweets durable, materialized timelines are recoverable cache |
| Filter blocked / muted accounts on read | Cost ceiling: storage growth bounded — timelines are capped lists |

### API contract

```
GET  /home_timeline?cursor=<tweet_id>&limit=50    -> { tweets[], next_cursor }
POST /tweet                  { idempotency_key, body, media[] } -> { tweet_id }
POST /follow                 { follower_id, followee_id }       -> { ok }
GET  /user/:id/tweets?cursor                                    -> { tweets[], next_cursor }
```

Notes: `home_timeline` is the hot path — must merge **push-delivered** entries (normal followees) with **pull-fetched** entries (celebrity followees) and re-sort. `cursor` is `(tweet_id, ts)` — opaque to client.

### Data model

```
tweets (Manhattan KV / Cassandra)
  tweet_id     (snowflake u64, PK)        # snowflake = (epoch_ms<<22 | shard<<12 | seq)
  author_id    (u64, index)
  body         (text, <=280 chars)
  media[]      (S3 URLs)
  created_at   (u64 — embedded in tweet_id, derivable)

user_tweets_by_author (secondary; or derived from tweet_id partitioning)
  partition_key: author_id
  sort_key:      tweet_id desc                    # for pull-on-read of celebrities

home_timeline (Redis cluster — per-user materialized list)
  key:   home:{user_id}
  type:  list of (tweet_id, score=created_at_ms)  # ZSET, capped at 800
  TTL:   30 days from last read; recompute from tweet store on miss

social_graph (Manhattan KV)
  followers_of:{user_id} -> set<user_id>          # for fan-out-on-write
  follows_of:{user_id}   -> set<user_id>          # for fan-out-on-read + UI

celebrities (small in-mem set, hot-reloaded)
  set<user_id> where follower_count > 10M         # the threshold
```

Access patterns this serves: (a) read materialized timeline by user — O(1) Redis ZRANGE; (b) on tweet, look up followers and push — O(F) writes; (c) on read, pull-fetch celebrity author tweets since cursor — O(C × pageDepth) reads where C = celebrities followed. Doesn't serve: full historical scroll past 800 (paginate from tweet store by author).

### Request-path layering (where this component lives)

```mermaid
flowchart LR
    U[User app] --> DNS[GeoDNS]
    DNS --> AC[Anycast IP]
    AC --> CDN[CDN POP — static + image]
    CDN --> WAF[WAF / abuse]
    WAF --> LB[Regional LB]
    LB --> GW[API Gateway: auth + rate]
    GW --> APP[Timeline service pod]
    APP --> RDS[(Redis: home:user_id ZSET)]
    APP --> TS[(Tweet store: Manhattan KV)]
    APP --> GRPH[(Social graph KV)]
    APP --> RANK[Ranker / mixer]
    style APP fill:#ffe680,stroke:#b58900,stroke-width:2px
    style RDS fill:#ffe680,stroke:#b58900,stroke-width:2px
```

**Where this concern lives** (timeline assembly):

| Layer | Sees | Latency cost | Blast radius | Best for |
|---|---|---|---|---|
| Edge / CDN | Static media, profile images | <1ms | Global | Image/avatar offload — never timeline JSON (per-user, uncacheable at edge) |
| API Gateway | user_id, auth, rate-limit | 1–3ms | Regional | Per-user QPS limit, JWT verify |
| App middleware (Timeline svc) | Push list + pull-fetch + merge | 20–50ms | Per-service | The actual work — this is where the design lives |
| Storage (Redis + Manhattan) | Materialized lists, tweet bodies | 1–5ms per call | Per-shard | The push artifact (Redis) + source of truth (Manhattan) |

Edge caching the timeline JSON is a trap — every user's feed is unique, so edge hit rate ≈ 0%. The win is at the **app+Redis** boundary, where the timeline is pre-materialized.

### Architecture

```mermaid
flowchart TB
    C[Client] -->|HTTPS, 150k QPS peak| LB[LB + API GW]
    LB --> TLS[Timeline svc]
    TLS -->|ZRANGE home:user_id| R[(Redis cluster\n32 shards)]
    TLS -->|MGET tweet bodies| TS[(Manhattan KV\ntweets)]
    TLS -->|"if user follows celebs:\nlatest by author"| TS
    POST[POST /tweet, 12k QPS] --> ING[Ingest svc]
    ING --> TS
    ING -->|publish tweet_id| KQ[Kafka: tweet-events]
    KQ --> FAN[Fan-out workers\n200 pods]
    FAN -->|"followers_of(author)"| GRPH[(Social graph)]
    FAN -->|"if author NOT celeb:\nZADD home:follower"| R
    FAN -.->|"if author IS celeb:\nNO-OP, pull at read"| R
```

NFRs: p99 200ms satisfied by Redis ZRANGE (1ms) + Manhattan MGET (5ms) + celebrity pull (10–30ms, capped at ~20 celebs followed) + serialization (2ms). Availability via Redis Sentinel + Cassandra RF=3. Eventual consistency via Kafka backlog (typical lag 1–3s, p99 <5s).

### Deep dive — 2 mechanisms

#### Deep dive 1: Fan-out-on-write vs fan-out-on-read (and the hybrid)

**1. Why does this mechanism exist?**

The naïve options both die. **Fan-out-on-read** (every timeline load fetches latest tweets from each followee): at 200 follows × 1ms Manhattan lookup = 200ms — busted before merge/sort. **Fan-out-on-write** (every tweet writes to all followers' Redis lists): at 100M followers (Obama), one tweet = 100M ZADDs. At ~10k ZADD/sec/Redis shard, that's 10k seconds = 2.7 hours of write-traffic from one celebrity tweet. The fan-out workers fall behind, every other tweet stalls, the queue grows without bound.

The hybrid mechanism: **classify each author by follower count**. Non-celebs (<10M followers) fan-out-on-write — the cost is bounded by their actual follower base, typical case is 200 writes per tweet. Celebs (≥10M followers) fan-out-on-read — they don't push at all; their tweets sit in the per-author tweet store. On timeline load for any user, look up *which celebrities they follow* (typically <20), pull each celebrity's latest tweets since the cursor, and merge with the pre-materialized list.

Upside: write cost capped at ~200 (typical) or zero (celebrity); read cost is constant + O(C × pageDepth) where C is small. Downside: read path now does extra Manhattan calls per request — a regression for users who follow many celebrities. But C is small in practice (median user follows 2 celebrities, not 200), and those reads can be parallelized + cached per-celebrity for ~1s.

**2. Walk it concretely.**

User `Alice` follows 198 normal users + Obama. Obama tweets at `t=0`; one of Alice's normal followees Bob tweets at `t=1s`.
...
sequenceDiagram
    ...
    Obama->>Ingest: POST /tweet "..."  (t=0)
    Ingest->>Manhattan: PUT tweets[T_obama] = {...}
    Ingest->>Kafka: publish {tweet_id: T_obama, author: obama}
    Kafka->>Fanout: consume
    Fanout->>Manhattan: is_celebrity(obama)? YES
    Note over Fanout: skip ZADD — pull-on-read

    Bob->>Ingest: POST /tweet "..."  (t=1s)
    Ingest->>Manhattan: PUT tweets[T_bob] = {...}
    Ingest->>Kafka: publish {tweet_id: T_bob, author: bob}
    Fanout->>Manhattan: is_celebrity(bob)? NO
    Fanout->>Manhattan: followers_of(bob) -> [alice, ...300 others]
    Fanout->>Redis: ZADD home:alice T_bob_score T_bob
    Fanout->>Redis: ZADD home:other_followers ...
    Note over Redis: home:alice now contains T_bob at top

    Alice->>TimelineSvc: GET /home_timeline  (t=3s)
    TimelineSvc->>Redis: ZREVRANGE home:alice 0 49
    Redis-->>TimelineSvc: [T_bob, T_prior_1, ...] (T_obama MISSING)
    TimelineSvc->>Manhattan: celebrities_followed_by(alice) -> [obama]
    TimelineSvc->>Manhattan: tweets_by_author(obama, since=alice.last_seen)
    Manhattan-->>TimelineSvc: [T_obama]
    TimelineSvc->>TimelineSvc: merge + sort by score
    TimelineSvc->>Manhattan: MGET [T_bob, T_obama, T_prior_1, ...]
    TimelineSvc-->>Alice: [T_obama, T_bob, T_prior_1, ...]
```

Step-by-step on the data:
```
t=0    Obama tweet T_obama  →  Manhattan PUT, Kafka publish, Fanout sees celeb → no ZADD
t=1s   Bob tweet T_bob      →  Manhattan PUT, Kafka publish, Fanout sees non-celeb → 300 ZADDs
t=3s   Alice GET            →  Redis ZREVRANGE home:alice → [T_bob, ...]  (cost: 1ms)
                              Manhattan lookup celebs Alice follows → [obama]  (cost: 1ms, cacheable)
                              Manhattan tweets_by_author(obama, since=...) → [T_obama]  (cost: 3ms)
                              Merge: heap-merge of (push_list, [obama_recent]) by score
                              MGET 50 tweet bodies → 5ms
                              Total: ~12ms — well under 200ms p99 budget
```
...
| Property gained | Cost paid |
|---|---|
| Write cost bounded by *actual* follower count of non-celebs (typically ≤10M) | Read path now does a celebrity-pull step per request (extra 5–15ms, scales with celebs-followed) |
| Single Obama tweet → 0 fan-out writes (vs 100M) | Two code paths to maintain: push-side fan-out worker + read-side merger |
| Backfill on new follow: just rebuild that one user's home from a tweets-by-author scan | Threshold (10M) is a tuning knob — bad value either underuses Redis or overloads it |
| Per-tweet promotion to "celebrity" is an idempotent set-add | If a normal user crosses 10M, in-flight fan-outs for that tweet still happen — accept some duplicated work or coordinate via a global "as-of" epoch |

**4. Failure modes the interviewer will drill into.**

| Question | What goes wrong | Mitigation |
|---|---|---|
| Q1: Fan-out worker falls behind during a viral cascade | Followers see stale timelines; backlog grows in Kafka | Auto-scale fan-out pods on Kafka lag; per-author rate-limit fan-out fairness (one author can't starve others); shed lowest-priority (inactive followers) first |
| Q2: User follows a celebrity *and* a normal user at the same instant, then loads timeline | Push entry for normal user is committed, celebrity pull hasn't been wired in yet | Read-side merger always runs the celebrity pull from the current `follows` snapshot, so newly-followed celebs appear on next refresh |
| Q3: Redis shard hosting home:alice crashes mid-load | ZRANGE fails | Redis Sentinel failover (3–10s); during failover, fall back to *pure pull* for that user — slower but correct |
| Q4: A non-celebrity tweets and then crosses the threshold mid-fan-out | Some followers got pushed, some won't; readers may double-count if mixer also pulls | Mixer dedups by tweet_id (it's a set-merge, not a list-concat). Crossing the threshold is rare (~1/day globally) and accepted as eventually-consistent |
| Q5: Celebrity goes inactive for a year then tweets once | Their followers have stale cursors; pull-on-read returns nothing or stale items | "Since cursor" pull is bounded — if cursor is older than 30d, pull only last N=200 by author; older history paginates from tweet store directly |

**5. How to derive it from first principles.**

1. **Read path budget**: 200ms p99. Subtract serialization (5ms), TLS (5ms), gateway (3ms) → app+storage has ~180ms.
2. **Naïve pull cost**: 200 follows × 1ms = 200ms → blows budget on its own. Reject.
3. **Naïve push cost**: avg 200 writes/tweet × 12k tweets/sec = 2.4M ZADD/sec — feasible. But worst case = max_followers × 12k = 1.2B writes/sec → reject for tail.
4. **Observation**: the cost asymmetry is *follower-count distribution*. Power-law: median ~50, p99 ~10k, p99.99 ~100M. The tail is a few thousand celebrity accounts.
5. **Hybrid**: classify by follower count. Pick threshold T such that (a) push cost is bounded above by T × tweets/sec and (b) read cost is bounded by C × pageDepth where C = celebs-followed (small).
6. **Threshold derivation**: T should balance write-cost-saved against read-cost-added. If Redis ZADD = 0.1ms and Manhattan tweets-by-author = 3ms, the break-even is when a celebrity's followers each save 30 worth of read-cost per write skipped. At C=20 celebs followed avg, T ≈ followers/(C × cost_ratio) ≈ 10M (rough, empirically tuned).
7. **Read merger**: heap-merge push list (sorted) + per-celeb pull lists (each sorted), keep top N by score. O((N + C·pageDepth) log C).
8. **You've derived the hybrid** without invoking Twitter's blog — it falls out of read-budget + write-distribution arithmetic.

**6. Where this shows up in production.**

- **Twitter's "Manhattan"** (real-time KV) + per-user timeline cluster: empirically ran a celebrity threshold around 10M followers; documented in their 2014 Manhattan post; fan-out workers processed ~2M writes/sec sustained pre-Musk.
- **Facebook News Feed**: doesn't use pure fan-out — uses **fan-out-on-read with aggressive caching** because Facebook's social graph is denser (avg ~340 friends) and ranking is ML-driven, so push doesn't help (ranker would re-shuffle). Documented in "Scaling Memcache at Facebook" SOSP 2013.
- **LinkedIn**: hybrid with a "feed mixer" — push for connections, pull for companies-followed and influencers; the mixer is the explicit merge stage. ~5B feed loads/day pre-2020.
- **Instagram**: pull-heavy because ranking is ML-driven (chronological is opt-in); they materialize a candidate set per user every few minutes rather than per-tweet push.

#### Deep dive 2: Per-user materialized timeline in Redis — structure, capping, hydration

**1. Why does this mechanism exist?**

If we tried to serve the timeline by querying `tweets WHERE author_id IN follows(user)` at read time, the planner has to scan 200 author partitions and merge-sort by created_at. Even with secondary indexes, that's 200 Manhattan calls per timeline load × 150k reads/sec = 30M Manhattan QPS on the read path alone. The materialized timeline solves it: the **fan-out worker** has already paid the cost of the join, exactly once per tweet, at write time. The reader just does `ZREVRANGE home:user_id 0 49` — one O(log N + K) operation on one Redis shard.

**Why Redis ZSET specifically.** Three properties needed: (a) ordered by score (created_at_ms), (b) bounded size (cap at 800 — older falls off), (c) O(log N) insert + O(log N + K) range read. Redis sorted sets are a **skip-list + hash table** combo. The skip-list gives ordered-by-score; the hash gives O(1) member existence check (for dedup). ZADD = skip-list insert (O(log N), N ≤ 800 → ~10 levels). ZREVRANGE 0 49 = walk top of skip-list from head, O(K=50). ZREMRANGEBYRANK trims the tail back to 800 — O(log N + M) where M = trimmed count, typically 1.

**Why Redis at all, not Cassandra.** Cassandra would work but the per-shard QPS economics are wrong. Cassandra targets ~5k writes/sec per node; Redis targets 100k+/sec on a single core. For 2.4M timeline writes/sec we'd need ~500 Cassandra nodes vs ~30 Redis nodes. Redis loses durability (in-memory) — accepted because timelines are *recoverable* (replay the fan-out from tweet store).

**2. Walk it concretely.**

Alice follows Bob, Carol, Dave. Timeline before is empty. Three tweets arrive.

```mermaid
stateDiagram-v2
    [*] --> Empty
    Empty --> S1: t=1000 ZADD home:alice 1000 T_bob_1
    S1 --> S2: t=1500 ZADD home:alice 1500 T_carol_1
    S2 --> S3: t=2000 ZADD home:alice 2000 T_bob_2
    S3 --> READ: GET /home_timeline → ZREVRANGE
    READ --> RES: returns [T_bob_2, T_carol_1, T_bob_1]
```

State of the skip-list inside `home:alice`:

```
After t=1000:  head → [score=1000, member=T_bob_1] → nil
After t=1500:  head → [1500, T_carol_1] → [1000, T_bob_1] → nil
               (skip-list inserted at correct sorted position; index O(log 1) = 0 levels above base)
After t=2000:  head → [2000, T_bob_2] → [1500, T_carol_1] → [1000, T_bob_1] → nil
ZREVRANGE 0 49:  walk from head → emit (T_bob_2, T_carol_1, T_bob_1)
                 Cost: 3 pointer derefs. K=3.
```

Capping example: after 800 entries, the 801st ZADD triggers `ZREMRANGEBYRANK home:alice 0 0` (remove lowest-score = oldest). Wrapped in a Lua script for atomicity. Memory per timeline: 800 × (~40 bytes per skip-list node + dict entry + member string) ≈ 32 KB. At 500M users hot, that's 16 TB — sharded across 160 shards × 100 GB. In practice most timelines aren't hot — LRU evicts inactive users, recompute on next visit.

Hydration: ZREVRANGE returned 50 tweet_ids → `MGET tweets:T_bob_2 tweets:T_carol_1 ...` against Manhattan. Manhattan partitions tweets by tweet_id (which embeds shard via Snowflake), so 50 MGETs hit ~50 distinct shards in parallel — each completes in ~2–3ms, the slowest determines the call latency (~5ms p99). Fan-out reads, not fan-in.

**3. The cost you're accepting.**

| Property gained | Cost paid |
|---|---|
| O(log 800 + 50) read on one Redis shard — easy fit in 1ms | In-memory storage: 16 TB across the cluster, ~$0.5M/month at AWS ElastiCache list price |
| Skip-list keeps tweets sorted by created_at without a separate sort step at read time | Skip-list nodes are bigger than array entries (~40 vs 16 B per element) — pay for the pointers |
| ZREMRANGEBYRANK bounds growth so memory is deterministic | Cap of 800 means a user who follows 200 accounts and scrolls fast can hit the cap and have to paginate from tweet store (a regression for power-users) |
| Recoverable: lose a Redis shard → replay last N days of fan-out from Kafka to rebuild | Replay during incident takes minutes-hours; during that window, affected users fall back to pure pull |

**4. Failure modes the interviewer will drill into.**

| Question | What goes wrong | Mitigation |
|---|---|---|
| Q1: Hot timeline — one celebrity follower has 50k tweets/day pouring in | Single Redis shard saturates on ZADD QPS | Per-user TTL on inactive timelines (drop), so only active users stay materialized; consistent-hash the user_id so load spreads; cap fan-out per user at e.g. 5k/sec |
| Q2: Tweet deleted after fan-out — entries linger in timelines | Stale tweet_ids in Redis; MGET on hydration returns null | Hydration filters nulls (`tombstone: true` row in Manhattan); never go back and ZREM from all timelines (write amplification disaster) |
| Q3: Redis cluster failover during a viral event | ZADDs block 3–10s; fan-out lag spikes | Kafka consumer pauses; auto-recovers on failover; worst-case readers see ~10s-stale timelines |
| Q4: 3am — Manhattan tweet-store is slow (compaction storm), MGET p99 → 80ms | Timeline load goes from 12ms → 90ms; close to budget | Hydration timeout 50ms per MGET; on timeout, return partial timeline (50 IDs, 40 hydrated) with a "loading…" placeholder for the rest. **Fail-open**: show what we have, never blank screen. Hard fail-closed would mean a popular tweet store outage takes down the entire timeline product — unacceptable. |
| Q5: Backfill on follow — Alice follows Carol who has 500 recent tweets | Naive backfill = 500 ZADDs synchronously, blocks the follow API | Async backfill job; the follow API completes immediately, backfill enqueues; in the meantime read-path merges Carol's recent tweets via pull (same celebrity merger reused) |

**5. How to derive it from first principles.**

1. **Per-request budget**: 50 tweet IDs returned, p99 < 200ms total. Storage must serve a sorted top-K query in <5ms.
2. **Sorted structure on disk**: every option (B-tree, LSM) costs at least one seek per query. Memory-resident is required for the latency budget.
3. **In-memory sorted structure**: balanced BST, skip-list, sorted array. Sorted array O(N) insert (kills write path). BST O(log N) but pointer-chasing-heavy and rebalance-on-insert. Skip-list O(log N) insert, simpler than BST, and Redis already implements it.
4. **Bounded size**: a feed isn't infinite — users scroll a few hundred tweets. Cap at 800 (~4 days at 200 follows × 1 tweet/follower/day). Anything older = paginate from tweet store.
5. **Atomic cap**: ZADD + ZREMRANGEBYRANK must be one operation, else race. Either Redis MULTI/EXEC (no isolation) or Lua script (atomic + single-threaded execution on Redis). Pick Lua.
6. **Memory budget**: 16 TB hot. Either denormalize tweet bodies into the ZSET (faster reads, 4× more memory) or store just tweet_ids and hydrate (cheaper, one extra MGET hop). Pick IDs-only because (a) tweet body changes break the design, (b) hydration is fan-out parallel so adds <5ms.
7. **Persistence**: lose the Redis cluster → rebuild from Kafka replay. Accept 30–60min recovery RTO because the loss is recoverable and saving every ZADD to disk would 10× the cost.
8. **You've derived a Redis-backed materialized timeline** without quoting Twitter's Manhattan post — it falls out of latency + memory + cost arithmetic.

**6. Where this shows up in production.**

- **Twitter timeline service**: Redis sorted sets, capped at ~800, fan-out via Manhattan triggers; covered in their 2017 "The Infrastructure Behind Twitter" post. Reported timeline read p50 around 5ms, p99 around 100ms.
- **Discord recent messages cache**: per-channel ZSETs in Redis sorted by message_id; capped at last 1000; cold reads spill to Cassandra. Documented in their 2017 "How Discord stores billions of messages" post — they read from a 3-tier cache (Redis → recent rows in Cassandra → wide rows for full history).
- **Reddit hot/new feeds**: per-subreddit ZSETs in Redis, scored by hotness (decayed score) or created_at; capped at ~1000; documented in their 2017 r2/r/announcements infra posts as backed by Cassandra + Redis sorted sets.
- **Instagram feed**: materialized candidate-set per user, refreshed every few minutes by an offline job; Redis-backed; read path picks top-N by ML score. Materialization is amortized rather than per-tweet because ranking dominates.

### Bottlenecks + scaling levers

| Scale point | First bottleneck | Lever |
|---|---|---|
| 10x QPS (1.5M reads/sec) | Redis cluster CPU on ZRANGE | Add shards (consistent-hash by user_id); each Redis core handles ~100k ops/sec |
| 100x QPS (15M reads/sec) | Manhattan MGET bandwidth on hydration | Per-tweet body cache in front of Manhattan (memcached); ~95% hit rate viable on the long tail |
| 10x writes (12M timeline writes/sec) | Fan-out worker throughput + Kafka partitions | Raise Kafka partition count (per-author hashing); add fan-out worker pods; lower the celebrity threshold (more authors skip push, less work) |
| 10x data (5B users, longer history) | Redis memory | Shrink timeline cap (e.g. 200 entries) and lean harder on pull-from-tweet-store pagination |
| Viral cascade (1 tweet → 1B reads in 5min) | Tweet store read fan-in on the *body* | Per-tweet body cache; CDN-cache the tweet detail page even though the timeline isn't cacheable |

### Trade-off matrix

| Option | Write cost | Read cost | Memory | Celebrity-safe |
|---|---|---|---|---|
| Pure fan-out-on-write | ❌ (Obama OOM) | ✅ (1 ZRANGE) | ❌ (16TB) | ❌ |
| Pure fan-out-on-read | ✅ (1 PUT) | ❌ (200 lookups) | ✅ (none) | ✅ |
| **Hybrid (chosen)** | ✅ (push for normal, skip for celeb) | ⚠️ (1 ZRANGE + C pulls) | ⚠️ (16TB push set) | ✅ |
| Pre-ranked materialization (Instagram-style) | ⚠️ (batch refresh) | ✅ (lookup) | ⚠️ | ✅ |

Hybrid is the answer for reverse-chronological feeds at Twitter's social-graph distribution. Pre-ranked is the answer when ML ranking dominates and chronological isn't required.

### Common follow-ups

1. **How do you handle a user with 100M followers (Obama)?** — Mark as celebrity (follower-count > 10M threshold). Their tweets skip fan-out entirely. Every follower's timeline read does an extra `tweets_by_author(obama, since=cursor)` lookup; the result merges into the materialized list before return. Cost: <5ms per celeb followed, bounded because users typically follow <20 celebs.
2. **How do you mix in promoted tweets without blocking the read path?** — Promoted tweets are a *side-channel* on the read path: parallel call to ad-mixer service while ZRANGE runs. Ad-mixer returns 0–3 promoted IDs; merger inserts them at fixed positions (e.g., rows 2 and 8). If ad-mixer times out at 30ms, return timeline without ads — never block timeline on ads.
3. **How do you backfill timelines when a user follows someone new?** — Sync API: just record the follow + return. Async job: read followee's last 200 tweets from `tweets_by_author`, ZADD each into follower's timeline (capped). During the window between follow and backfill complete, read-path's celebrity-merger logic also pulls the new followee — so the user sees the new followee's tweets immediately, even before backfill lands.
4. **How do you serve scroll past 800 tweets?** — Paginate from tweet store directly: for each followee, `tweets_by_author(followee, before=cursor)` and merge. Slower (200ms+) but used only by the small fraction who scroll deep. The 800 cap is "what's cached", not "what's available".
5. **What happens on tweet delete?** — Mark tweet_id as deleted in Manhattan; *don't* go back and ZREM from every materialized timeline (that'd be the original fan-out problem in reverse). On hydration, MGET returns the tombstone, and the timeline service drops it from the response. Net effect: deleted tweets disappear from views without the storage system doing fan-out-on-delete.

### What top-tier looks for

- **Mid-tier says**: "Use fan-out on write — push the tweet to all followers' timelines."
- **Staff-tier says**: "Pure fan-out fails at the Obama case (100M followers × 12k tweets/sec at the tail). The signal is *follower-count distribution is power-law*; the fix is a threshold-based hybrid — push for the typical user, pull for celebrities, merge at read time. The threshold is itself derived from comparing fan-out cost to merge cost: at ~10M followers, the math flips. Twitter publicly ran this; LinkedIn does it via their 'feed mixer'; Facebook punts to fan-out-on-read entirely because their social graph is denser and ranking dominates anyway."

The difference: mid-tier names a pattern; staff-tier *derives why neither extreme works* from the workload's actual distribution, then names the lever (threshold) and how to tune it.

### Diagnostic questions (self-quiz)

1. **Why is fan-out-on-read alone insufficient at p99 200ms?** — wrong: "Manhattan is slow." Right: 200 sequential lookups × 1ms = 200ms, exhausts the read budget before merge/sort. Parallelism doesn't help because the tail latency of N parallel calls grows with N.
2. **Why is fan-out-on-write alone insufficient?** — wrong: "It's slow." Right: a celebrity tweet's write cost = follower_count; at 100M followers, one tweet generates 100M ZADDs, ~3 hours of work on the cluster, backing up everyone else's fan-out behind it.
3. **Why a sorted set (ZSET) and not a list?** — wrong: "Redis lists don't sort." Right: tweets arrive *out of order* from fan-out workers (different shards, different latencies); ZSET sorts by score (created_at) on insert, so ZREVRANGE returns chronologically-correct top-K regardless of fan-out arrival order.
4. **Why cap at 800, not unlimited?** — wrong: "memory." Right: 800 covers ~4 days at avg follow-count, beyond which (a) users rarely scroll, (b) data fits in 32KB/user which fits the cluster budget, (c) trimming is O(log N) ZREMRANGEBYRANK. Unbounded growth is both a cost and a per-shard hot-key problem.
5. **What happens to a follower who follows a user that *just* crossed the celebrity threshold mid-fan-out?** — wrong: "They miss the tweet." Right: the tweet either landed in their materialized timeline (push happened) or will land via the celebrity-merger on next read (pull happens). The mixer dedups by tweet_id, so even if both happen, the user sees it exactly once.
